...
if (import.meta.url === `file://${process.argv[1]}`) {
	// Wrap in an async IIFE so no await appears at the module's top level.
	// Top-level await is valid ES module syntax but tsx warns if the process
	// exits while a top-level await is still pending.
	;(async () => {
