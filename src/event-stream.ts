...
export class EventStream<T, R> implements AsyncIterable<T> {
	...
	constructor(
		...
		// Suppress "unhandled rejection" warnings when .result() is never called.
		// Callers that do call .result() still receive the rejection normally.
	...
	[Symbol.asyncIterator](): AsyncIterator<T> {
		return {
			next: () => {
				...
				// If there was an error, reject (check before done so fail() always throws)
				if (this.error) {
